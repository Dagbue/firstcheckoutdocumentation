...
import { ChevronRight, Home, FileText, Settings, Code, CreditCard, Shield, HelpCircle, Phone, Key, Package, Globe, Wordpress, Link2, TestTube, Bug } from 'lucide-react';
  ...
  },
  {
    ...
    children: [
      { 
        ...
        path: '/api-sdk',
        description: 'Direct API calls and custom development',
        icon: Code
      },
      { 
        id: 'npm-package', 
        label: 'NPM Package', 
        path: '/npm-package',
        description: 'JavaScript frameworks integration',
        icon: Package
      },
      { 
        id: 'cdn-script', 
      ...
      { 
        ...
        path: '/wordpress',
        description: 'WooCommerce and WordPress sites',
        icon: Wordpress
      },
      { 
        id: 'payment-links', 
        label: 'Payment Links (No-Code)', 
        path: '/payment-links',
        description: 'Create payment links without coding',
        icon: Link2
      }
    ]
  },
                ...
                {item.children ? (
                  <>
                    <button
                      ...
                      className={`w-full group flex items-start px-3 py-3 text-sm font-medium rounded-lg transition-all duration-200 ${
                      ...
                      <div className="mt-2 ml-8 space-y-1">
                        {item.children.map((child) => (
                          <Link
                            key={child.id}
                            to={child.path}
                            className={`w-full group flex items-start px-3 py-2 text-sm rounded-lg transition-all duration-200 ${
                              isActive(child.path)
                                ? 'bg-bank-blue bg-opacity-10 text-bank-blue border-l-2 border-bank-gold'
                                : 'text-gray-600 hover:bg-gray-50 hover:text-gray-800'
                            }`}
                          >
                            <child.icon className="mr-3 h-4 w-4 flex-shrink-0 mt-0.5" />
